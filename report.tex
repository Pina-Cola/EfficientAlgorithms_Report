...
If the substring is more than one terminal symbol, the parser iterates over the rules and checks if there is a rule which can generate the non-terminal symbol and the substring by recursively calling itself on its two subparts. This loop also takes O(m) time.
