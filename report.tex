...
\begin{minipage}{0.5\textwidth}
The $O$-notation results from Section \ref{systemdesign} show, that the naive approach has a runtime of $O(2^n)$ and the topdown approach has a upper bound runtime of $O(n!)$ and the bottomup parser has an upper bound runtime in $O(n^3)$.
