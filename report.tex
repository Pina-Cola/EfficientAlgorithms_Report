...
This algorithm has a runtime in $O(n^3)$ because it contains three nested loops. The outer loop and the middle loop iterate $n$ times and the inner loop iterates $j$ times, which depends on $n$, too. Because of this, the number of iterations is $n \cdot n \cdot n$ which is $O(n^3)$.
...
The results of the experiments confirm the runtime calculation of $O(2^n)$ in Section \ref{naiveruntime}. Small deviations for smaller input numbers can happen, because the constants are not considered.
...
The results of the experiments do not confirm the runtime calculation of $O(n^3)$ in Section \ref{lineartopdown}. This is further discussed in Section \ref{results_linear}.
...
\begin{minipage}{0.5\textwidth}
The $O$-notation results from Section \ref{systemdesign} show, that the naive approach has a runtime of $O(2^n)$ and the topdown approach has a upper bound runtime of $O(n^3)$ and the bottomup parser has an upper bound runtime in $O(n^3)$.
...
The upper bound runtime of the modified topdown parser for linear grammars is faster than the runtimes of the parser for grammars in CNF. The topdown runtime is in $O(n^3)$ and the modified topdown approach should be in $O(n^3)$. In the linear approach are one instead of two recursive call per execution made, because the linear grammars have at most one nonterminal symbol in the body of each rule. The recursive call needs only to be made for the nonterminal symbols. 
