...
% TODO: definition and step by step into CNF
...
% TODO: definition und step by step
...
\\
% TODO: mehr erklären und mit Beispielen!
...
The top-down method is an improves version of the naive method (section \ref{naive}). This algorithm works recursive, too. In this algorithm another array is used, which contains the values \texttt{true}, \texttt{false} and \texttt{null}. If one of the values is not \texttt{null}, the next recursive call is not executed.
...
The runtime analysis of the \texttt{parseTD} method is analog to the \texttt{parseNaive} runtime analysis. Both algorithms have a runtime in $O(n^2)$.
...
\ \\
Seeing those runtimes one could think that the naive and the topdown approach are equally efficient. Considering the differences in the algorithms and the results of the experiments it gets shown that this is not the case. The O-notation runtimes are the upper boundary for the runtime. 
